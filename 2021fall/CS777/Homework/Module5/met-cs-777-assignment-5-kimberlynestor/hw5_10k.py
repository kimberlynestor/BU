...
# n_word = 20000
# if 20k doesn't work please try 2k, i haven't been able to get 20k to work, keep getting a memory error
n_word = 2000
# n_word = 200
...
def get_data(data_path):
    ...
    df_output = df_info_vec.select(['docID', 'label', 'tf_vec_norm'])
    df_output = df_output.withColumn('tf_vec_norm', to_vec('tf_vec_norm'))
...
# init pca
pca = PCA(k=int(n_word/2), inputCol="tf_vec_norm", outputCol="features")
...
"""
Discussion:
i used the principal component analysis (pca) to reduce dimensionality of the 
tf vector. i used this technique because it allows reduction of a dataset but 
maintains high accuracy because it shifts the variance of the data set to the 
left. this shift causes the variables to the right to contain less information 
about the dataset, so they can be dropped from the dataset to obtain the new 
preferred dimension without losing information.

pca was performed on the test and train dataframe after data normalization. 
the resulting logistic regression f1 score after dimensionality reduction was 
identical to the score obtained without reduction: 0.969900501154989. as for 
svm the score prior to reduction was 0.9605179691254087 and after reduction 
there was a change to 0.9698737995006702, which is still very close to the 
original and is actually a higher score, meaning there were more correct 
predictions after dimensionality reduction. this is a great example of no loss 
of accuracy after reduction using pca, and the main reason i used this technique. 
it is also very scalable to large datasets.
"""